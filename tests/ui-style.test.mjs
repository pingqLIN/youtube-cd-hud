...
test('shows the tracklist explicitly and uses the redesigned compound controls', () => {
  ...
  assert.match(source, /createControlButton\(t\('useTrackId'\)/);
  assert.match(source, /createControlButton\(t\('searchMixesDb'\)/);
  ...
  assert.match(source, /headerTitle\.textContent\s*=\s*t\('tracklist'\)/);
