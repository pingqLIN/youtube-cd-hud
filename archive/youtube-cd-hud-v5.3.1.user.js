// ==UserScript==
// @name         YouTube CD Album & HUD Overlay (with Tracklist + 1001Tracklists) v5.3.1
// @namespace    http://tampermonkey.net/
// @version      5.3.1
// @description  修復 Trusted Types、YouTube 換片競態、1001Tracklists 新版曲目結構、重試按鈕與封面配色
// @author       You
...
(function () {
    ...
    function getResponseDocument(response) {
        const candidates = [response && response.response, response && response.responseXML];
    ...
    function fetchTracklistFrom1001(title, videoId, force = false) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (resp) {
                ...
                const doc = getResponseDocument(resp);
                if (!doc) {
                    markSearchError('Search response was not returned as an HTML document.');
                    return;
                }
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (resp2) {
                        ...
                        const tracklistDocument = getResponseDocument(resp2);
                        if (!tracklistDocument) {
                            markSearchError('Tracklist response was not returned as an HTML document.');
                            return;
                        }
