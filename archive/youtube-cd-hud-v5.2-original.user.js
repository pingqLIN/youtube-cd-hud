// ==UserScript==
// @name         YouTube CD Album & HUD Overlay (with Tracklist + 1001Tracklists) v5.2
// @namespace    http://tampermonkey.net/
// @version      5.2
// @description  自動擷取封面配色、拖曳、縮放、雙擊播放/暫停；支援描述及 1001tracklists 曲目切換，帶狀態燈號與重試功能
// @author       You
...
(function () {
    ...
    if (window.trustedTypes && window.trustedTypes.createPolicy) {
        try {
            ttPolicy = window.trustedTypes.createPolicy('yt-cd-hud-v5', {
                createHTML: (input) => input,
            });
        } catch (e) {
            console.warn('[CD HUD] TrustedTypes policy creation failed, fallback to raw string.', e);
        }
    }

    function trustedHTML(htmlString) {
    ...
    function parseTimestampToSeconds(timeText) {
        const parts = trim(timeText).split(':');
    ...
    function parseDescriptionTracks() {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (title) {
                tracksFromYouTube.push({ time: parseTimestampToSeconds(match[1]), title });
                found++;
            }
        }
    ...
    function fetchTracklistFrom1001(title, videoId, force = false) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (resp) {
                if (resp.status !== 200) {
                    console.warn(`[CD HUD] Search failed with status ${resp.status}`);
                    searchState = 'error';
                    updateStatusLight();
                    return;
                }
                ...
                const linkEl = doc.querySelector('a[href*="/tracklist/"]') ||
                               doc.querySelector('a.tracklist-link') ||
                               doc.querySelector('a[href^="/tracklist/"]');
                if (!linkEl) {
                    console.log('[CD HUD] No tracklist link found on search page.');
                    searchState = 'error';
                    updateStatusLight();
                    return;
                }
                ...
                if (!href.startsWith('http')) {
                    href = 'https://www.1001tracklists.com' + href;
                }
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (resp2) {
                        if (resp2.status !== 200) {
                            console.warn(`[CD HUD] Tracklist page fetch failed, status ${resp2.status}`);
                            searchState = 'error';
                            updateStatusLight();
                            return;
                        }
                        ...
                        if (tracks && tracks.length > 0) {
                            ...
                            console.log('[CD HUD] No tracks parsed from 1001 page.');
                            searchState = 'error';
                            updateStatusLight();
                        }
                    },
                    onerror: function (err) {
                        console.error('[CD HUD] Error fetching tracklist page:', err);
                        searchState = 'error';
                        updateStatusLight();
                    }
                });
            ...
            onerror: function (err) {
                console.error('[CD HUD] Error searching 1001tracklists:', err);
                searchState = 'error';
                updateStatusLight();
            }
        });
    ...
    function parseTracklistHtml(html) {
        ...
        if (rows.length) {
            rows.forEach(row => {
                const timeEl = row.querySelector('.tlTime, .tl-time, .time, .timestamp, .duration');
                const titleEl = row.querySelector('.tlTrack, .tl-track, .track-name, .title, .track, .track-title');
                if (timeEl && titleEl) {
                    const timeStr = trim(timeEl.textContent);
                    const title = trim(titleEl.textContent);
                    if (timeStr && title) {
                        const seconds = parseTimestampToSeconds(timeStr);
                        tracks.push({ time: seconds, title });
                    }
                }
            });
        }
        ...
        if (!tracks.length) {
            ...
            while ((match = regex.exec(html)) !== null) {
                const timeStr = trim(match[1]);
                const title = trim(match[2]);
                if (title) {
                    const seconds = parseTimestampToSeconds(timeStr);
                    if (!tracks.some(t => t.time === seconds && t.title === title)) {
                        tracks.push({ time: seconds, title });
                    }
                }
            }
    ...
    function isControlTarget(target, hud) {
        let node = target;
        while (node && node !== hud) {
            if (node.classList && node.classList.contains('hud-control-button')) return true;
    ...
    function bindHudDragging(hud, player) {
        ...
        const startDrag = (clientX, clientY) => {
            ...
            document.addEventListener('touchmove', moveHudTouch, true);
    ...
    function bindTracklistDragging(element, container) {
        ...
        const startDrag = (clientX, clientY) => {
            ...
            document.addEventListener('mouseup', stopDragging, true);
            document.addEventListener('touchmove', movePanelTouch, true);
            document.addEventListener('touchend', stopDraggingTouch, true);
        };
    ...
    function createHud(player) {
        ...
        if (!hud) {
            ...
            statusLight.addEventListener('click', retrySearch);
            ...
            linkBtn = createControlButton('🔗', '開啟 1001 頁面', () => {
                if (tracklistUrl1001) {
                    window.open(tracklistUrl1001, '_blank');
                }
            });
            linkBtn.style.display = 'none';
    ...
    function initialize() {
        ...
        if (videoId && videoId !== lastVideoIdFor1001) {
            ...
            lastVideoIdFor1001 = videoId;
            lastSearchTitle = '';
            if (tracksFromYouTube.length) setActiveSource('youtube');
        }
