...
- **換影片，接著找歌單**：設定頁的「自動搜尋」控制切換影片後的 1001Tracklists 比對，預設開啟。
- **找到資料，自選切換方式**：開啟「優先採用 1001」後，搜尋成功會自動切換來源；預設關閉，保留目前來源。
- **需要更多線索，再補充搜尋**：可手動查詢 MixesDB、TrackId.net，並在獨立結果之間切換，保留播放位置。

此功能比對既有曲目資料與時間戳，不是聽音辨識。能否找到完整曲目取決於來源資料；1001Tracklists 要求瀏覽器驗證時，可透過 OPEN 1001 完成驗證。

[安裝 Chrome 擴充功能](#方案-b--chrome-擴充功能) · [使用 Tampermonkey](#方案-a--tampermonkey) · [深入了解來源與比對](#曲目來源與比對方式)
...
- [專案狀態](#專案狀態)
- [這個專案在做什麼](#這個專案在做什麼)
- [自動搜尋曲目](#自動搜尋曲目跟著播放進度聽)
- [options.html 設定頁](#optionshtml把播放器調成你的樣子)
- [版面編輯器預覽](#版面編輯器預覽)
- [如何選擇安裝方式](#如何選擇安裝方式)
- [快速開始](#快速開始)
- [曲目來源與比對方式](#曲目來源與比對方式)
...
## options.html：把播放器調成你的樣子

Chrome 擴充功能的 [options.html 設定頁](extension/options/options.html) 將**資料來源、外觀與版面編輯**集中在同一個控制台。從 Chrome 擴充功能選單開啟「選項」，或快速雙擊工具列圖示，就能一邊調整、一邊查看預覽。

<table>
<tr>
<td width="50%"><img src="docs/assets/readme/youtube-cd-hud-options-overview.png" width="100%" alt="真實設定頁截圖：來源搜尋、外觀控制與即時預覽" /><br /><sub>來源搜尋與外觀設定</sub></td>
<td width="50%"><img src="docs/assets/readme/youtube-cd-hud-options-configured.png" width="100%" alt="真實設定頁截圖：黃色配色、115% 唱片倍率與92%不透明度" /><br /><sub>黃色配色、115% 唱片倍率、92% 面板不透明度</sub></td>
</tr>
</table>

*以上是既有 5.8.3 版設定頁的實際截圖；目前版本增加了版面編輯控制，配置有所不同。*

| 調整項目 | 對你的播放體驗有什麼幫助 |
| --- | --- |
| 來源與搜尋 | 啟用需要的資料來源、切換自動搜尋與優先採用 1001，並設定 5～30 秒逾時及 1～10 個候選頁上限。 |
| HUD 外觀 | 顯示或隱藏唱片與跳曲控制，調整字級、唱片倍率、面板不透明度、字型與重點色。 |
| Live Monitor 版面 | 拖移元件、選取後編輯屬性，載入內建配置，再用相對尺寸與群組縮放調整版面。 |
| 自訂 CSS | 為有額外需求的使用者提供外觀覆寫入口。 |
| 儲存並套用 | 先在預覽確認，再保存設定與版面；A／B／C 另提供需手動 SAVE 的本次瀏覽器工作階段暫存。 |

## 版面編輯器預覽

**選到哪個元件，就調整哪個元件。** 曲名、時間、唱片、來源按鈕與曲目面板可以各自定位與設定，讓 HUD 配合影片畫面，而不是遮住你想看的內容。

![實際編輯器局部截圖：選取 Panel base 後調整背景、邊框、圓角、包覆留白與效果](docs/assets/readme/youtube-cd-hud-layout-editor-controls.png)

*既有實際操作截圖：選取面板底座，畫布下方顯示對應屬性。圖中的 ABS 模式與曲名是當時的編輯狀態；新版面預設為 REL %。*

| 功能 | 實際可以做什麼 |
| --- | --- |
| 元件參數 | 圖中底座提供背景色、邊框、圓角、包覆留白、陰影與側邊強調線。文字元件另有字型、字級、文字色彩、透明度與對齊；可用參數依元件用途而定。 |
| 圖層順序 | 啟用 Z axis 後，以 −99～99 調整元件前後順序；底座會包覆內容並保持在下層。 |
| 保持比例 | Size 依當前寬高比調整單一元件；SCALE % 可一同縮放選取元件、重疊群組或全部元件，保持相對位置。此行為由縮放操作提供，沒有獨立的比例鎖定開關。 |
| 曲目展示 | 獨立設定曲目面板與文字外觀，長列表可捲動；播放時醒目標示目前曲目，曲名元件可啟用跑馬燈。 |
| 空位與碰撞處理 | 新增元件時搜尋空位。拖曳造成同層重疊時，確認可自動配置 Z 軸；取消則會把衝突元件移出面板。 |
| 保存位置與配置 | 設定頁須按「儲存並套用」；在 YouTube 播放端拖移 HUD 元件，放開後會自動保存位置。 |
