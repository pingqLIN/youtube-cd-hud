...
![實際設定頁截圖：黃色配色、完整曲名區域與唱片配置](docs/assets/readme/youtube-cd-hud-options-configured.png)

*改用排版完整的既有設定頁截圖：黃色重點色、115% 唱片倍率與 92% 面板不透明度。此圖為 5.8.3 版的外觀設定預覽，不是目前版面編輯器的最終畫面。*

| 功能 | 實際可以做什麼 |
| --- | --- |
| 元件參數 | 面板底座提供背景色、邊框、圓角、包覆留白、陰影與側邊強調線。文字元件另有字型、字級、文字色彩、透明度與對齊；可用參數依元件用途而定。 |
